...
public class EnsembleManagerSettingsPanel extends JPanel implements SettingsPanel {
    ...
    private void initComponents() {
        ...
        btnAddManager.addActionListener(e -> {
            addManagerPanel(new Person());
            tbpManagers.setSelectedIndex(tbpManagers.getTabCount() - 1);
        });
        btnRemoveManager.addActionListener(e -> removeManagerPanel());
    }
    ...
    private EnsembleManagerPanel addManagerPanel(Person person) {
        ...
        panel.addInfoPanelListener(event -> {
    ...
    public void reloadSettings() {
        final DefaultDataManager manager = DefaultDataManager.getInstance();
        final List<Person> managers = manager.getDatabaseEntities("SELECT p FROM Person p LEFT JOIN p.personDatas as pd " +
